...
func Test_sqlMaker_GetSelect(t *testing.T) {
	...
	assert.Equal(t, res, "SELECT `id`, `Name`, `a` FROM `tname`")
}
...
func Test_sqlMaker_GetSelect_排序(t *testing.T) {
	...
	assert.Equal(t, res, "SELECT `id`, `Name`, `a` FROM `tname` ORDER BY `id` DESC, `name` ASC")
}
...
func Test_sqlMaker_GetSelect_条件(t *testing.T) {
	...
	assert.Equal(t, res, "SELECT `id`, `Name`, `a` FROM `tname` WHERE id > 10")
}
...
func Test_sqlMaker_GetSelect_Skip或Take大于0(t *testing.T) {
	...
	assert.Equal(t, res, "SELECT `id`, `Name`, `a` FROM `tname` LIMIT 10,100")
}
